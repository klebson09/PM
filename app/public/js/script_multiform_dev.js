...
function validarCamposEtapa(current_fs){
	...
	var senha = currentFieldset.getElementsByClassName("validarSenha")[0];
	var csenha = currentFieldset.getElementsByClassName("validarSenha")[1];
	var i = 0;
	var j = 0;
	...
	for(i=0;i<inputs.length;i++){
		if(!validarCampoEstaVazio(inputs[i])){
			...
			msgs.push(inputs[i].getAttribute("placeholder").replace("*","")+" está vazio!");
		} else {
...
function validarSenha(senha, csenha){
	...
	if(senha != undefined && csenha != undefined){
		if(validarCampoEstaVazio(senha) && validarCampoEstaVazio(csenha)){
			...
			msgs.push(senha.getAttribute("placeholder")+" está vazio!");
			msgs.push(csenha.getAttribute("placeholder")+" está vazio!");
		}
